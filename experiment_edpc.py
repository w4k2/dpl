...
# Generate data
n_samples = 1000
X, y, ns = snakeway(n_samples=n_samples, n_centroids=12)
...
models = {
    'none': EDPC(s, entropy_mode='none', **cc),
...
d = 4096
prober = np.linspace(-dd * model.inner_radius,
                        dd * model.inner_radius, d).reshape(d, 1)
...
az = ax[-4]
az.hist(X[y==0], range=(-dd * model.inner_radius,
                      dd * model.inner_radius), 
            bins=1000, log=True,
            color='black')
az.hist(X[y==1], range=(-dd * model.inner_radius,
                      dd * model.inner_radius), 
            bins=1000, log=True,
            color='tomato')
...
for iter in range(n_iters):
    ...
    prober2 = np.linspace(-dd2 * model.outer_radius,
                            dd2 * model.outer_radius, d).reshape(d, 1)
